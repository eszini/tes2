 ...
 *
 * 	./tes_cpar06 -inp=h01  -out=h01_tkn
 *
 * 	./tes_cpar06  h01  h01_tkn
 *
 *
 ...
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk                    ( sale en formato columnar, con tags donde aplique )
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk  -m                ( pasa todo a minuscula para eliminar el problema del case sensitive )
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk  -s -m             ( aplica solo a la "frase" despues de marca t3 )
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk  -s -m -nvd=1      ( agrega la frase procesada y nro de token en la salida )
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk  -s -m -f          ( genera salida en formato frase, con tags )
 *
 * 	./tes_cpar06 h01 h01_tag  -mrk=f05_t1.mrk  -v -s -m -e       ( salida formato sentencia, con tags, en minuscula, con EOL, muestra proceso por pantalla )
 *
 *
 *	9) Para detectar negacion con gramatica tipo negex ampliado
 *
 *	./tes_cpar06 h01 h01_txt  -dng=nga -grm=parser_nga.txt  -v -f
 *
 *
 *	10) Utilizar el diccionario
 *
 *	./tes_cpar06 h01 h01_txt  -dic=diccio01.txt -v               ( genera archivo 'diccio01'.dif ... con palabras que no encontro en el diccionario )
 *
 *
 */
...
int	proceso_principal()
{
	...
	while (fgets(b1, MAXB, hfinp) != NULL)
	{
		...
		/*
		 * Termino de tokenizarm, tenemos:
		 * q_tk		cantida de tokens
		 * tk[N]	tabla con tokens
		 *
		 * MMM
		 *
		 * Atencion:
		 * Busca directamente en la linea de entrada ... el flag -m solo tiene
		 * efecto en como imprime en archivo de diferencias, no en la busqueda.
		 *
		 */
		...
		if (gp_diccionario )
		{
			for (j=0; j< q_tk; j++)
			{
				if ( es_word(tk[j]) )
				{
					if ( !bd_buscar(pasar_a_minusc(tk[j])) )
						fprintf (hfou2,"%s\n",tk[j]);
				}
			}
		}
		 ...
		 * tokens en vector.
		 * Se procesa la linea entera con
		 * -dng=lex   lex & yacc
		 * -dng=nga   negex ampliado
		 *
		 */
		...
		/* parseo la linea entera con negex ampl */
		if (gp_tipneg == 2)
		{
			tok=nnparse();
			if (tok)
	...
	{
		printf ("Cant de lineas procesadas %d\n",q_lin);
		printf ("Cant de neg detectadas    %d\n",gc_si);
		printf ("Cant de neg no detectadas %d\n",gc_no);
		printf ("\n\n\n");
	}
